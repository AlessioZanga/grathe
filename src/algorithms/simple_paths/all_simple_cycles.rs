...
///
/// Find all cycles in a given graph. For undirected graphs, this algorithm [^1] ... TODO: .
/// For directed graphs, this algorithm [^2] returns all elementary circuits (i.e. *cycles*)
/// present in a graph. The graph can contain self-edges and can also be disconnected.
///
/// [^1]: [Paton, K. (1969). An algorithm for finding a fundamental set of cycles of a graph.](https://scholar.google.com/scholar?q=an+algorithm+for+finding+a+fundamental+set+of+cycles+for+an+undirected+linear+graph)
